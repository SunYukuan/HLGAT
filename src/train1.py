...
print("eps {:} | layer_num {:} | tr_ratio {:} | dropout {:}".format(
         args.eps, args.layer_num, args.train_ratio, args.dropout))
device = 'cuda'
accs = []
for i in range(20):
    ...
    print("eps {:} | layer_num {:} | tr_ratio {:} | dropout {:} | pl {:} | ph {:}|".format(
                args.eps, args.layer_num, args.train_ratio, args.dropout, args.p_l, args.p_h))
    # main loop
    dur = []
    los = []
    loc = []
    ...
    if args.dataset in ['cora', 'citeseer', 'pubmed'] or 'syn' in args.dataset:
        ...
        print(acc)
        accs.append(acc)
    #torch.save(net,'====\n{} - {} - {}.pth'.format(args.dataset, args.train_ratio, args.layer_num))
print('==========================', np.mean(accs))